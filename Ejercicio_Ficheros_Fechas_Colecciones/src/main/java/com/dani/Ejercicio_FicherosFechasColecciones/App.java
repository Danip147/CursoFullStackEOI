...
public class App {
	...
	static LocalDate fechaNacimiento = null;
...
//			e.printStackTrace(); // mensaje ERROR por defecto del sistema
	...
	public static void juntarTodosLosFicheros() {
	...
	public static void saludoBienvenida(String dni) {
		String nombre = "";
		String codigoPais = "";
	...
	public static void comprobarFechas(LocalDate fechaNacimiento) {
		...
		if (fechasUnicas.size() > 1) {
			...
			// aqui tienes que buscar el saldo de la persona que coincide con la fecha de nacimiento
...
//					+ fechaNacimiento.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
	...
	public static void productoAdecuado(Integer saldo) {
		for (String producto : productos) {
			...
			long edadCliente = ChronoUnit.YEARS.between(fechaNacimiento, LocalDate.now());
	...
	public static void main(String[] args) {
		...
		juntarTodosLosFicheros();
		saludoBienvenida(dni);
		comprobarFechas(null);
		Integer saldo = saldoTotal();
		System.out.println("Los mejores productos para ti son: ");
		productoAdecuado(saldo);
